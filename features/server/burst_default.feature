Feature: Burst control is on by default (isaac-udnm follow-up)
  ...
  without it. It is now ON by default with the zanebot values — threshold 30
  unauthenticated responses in a 60 s window, 10 min cooldown, throttle on,
  ...
  @wip
  Scenario: with no burst config at all, thirty unauthenticated requests trip a burst and the client is throttled
  ...
  Scenario: an explicit knob overrides its default and the others keep theirs
    Given config:
      | key                  | value |
      | http.burst.threshold | 5     |
    And the Isaac server is started
    When the client sends GET "/.env" with header "X-Forwarded-For: 203.0.113.9" 5 times
    Then the log has entries matching:
      | level | event                  | client      | count | window-ms |
      | :warn | :server/burst-detected | 203.0.113.9 | 5     | 60000     |
    When the client sends GET "/.env" with header "X-Forwarded-For: 203.0.113.9" 1 times
    Then the response status is 429
  ...
  Scenario: turning burst control off on hot reload releases a throttled client
    ...
    And the Isaac server is started
    When the client sends GET "/.env" with header "X-Forwarded-For: 203.0.113.9" 31 times
    Then the response status is 429
